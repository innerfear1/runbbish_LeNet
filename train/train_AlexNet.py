...
def main():
    ...
    data_transformer = {
        'train': transformers.Compose([
            # 将输入图片大小改成224*224*3
            transformers.RandomResizedCrop(227),
    ...
    best_acc = 0.0
    # 训练结束的模型参数结果保存位置
    save_path = 'pth/AlexNet.pth'
    # 定义画图对应的AlexNet网络的准确率和损失
    AlexNet_loss = []
    AlexNet_acc = []
    ...
    for epoch in range(epochs):
        ...
        for step1,data in enumerate(train_bar):
            ...
            # 正向传播得出输出
            output = model_AlexNet(images.to(device))
            # 按行最大输出对应的索引值
        ...
        with torch.no_grad():
             # 通过网络预测出结果
            val_output = model_AlexNet(val_images.to(device))
            # 计算此参数用测试集图片的对应的损失值
            ...
            # 将对应的损失放入损失的列表中
            AlexNet_loss.append(val_loss)
            # 将tensor类型数据转成float类型并加入准确率对应的列表中
            AlexNet_acc.append(val_acc/val_labels.size()[0])

        # 如果准确率大于基准的准确率0，就保存此训练参数文件
        if (val_acc/val_labels.size()[0]) > best_acc:
            best_acc = val_acc/val_labels.size()[0]
            torch.save(model_AlexNet.state_dict(),save_path)
    # 打印输出训练结束
    print('train_finished')

    # 定义和写入训练轮数的列表
    epoch_ls = list(range(1, epochs + 1))
    AlexNet_acc = torch.tensor(AlexNet_acc, device='cpu')
    AlexNet_loss = torch.tensor(AlexNet_loss, device='cpu')
    # 定义画图的画布大小
    fig, ax = plt.subplots(figsize = (7, 5))
    # 在画布中画出AlexNet损失曲线
    AlexNet_loss_plt, = ax.plot(epoch_ls, 
                             AlexNet_loss, 
                             color = 'red', 
                             linewidth = 1.5, 
                             linestyle = ":", 
                             ...
                             label = 'AlexNet损失')
    # 在画布中画出AlexNet的准确率曲线
    AlexNet_acc_plt, = ax.plot(epoch_ls,
                            AlexNet_acc,
                            color = 'orange',
                            linewidth = 1.5,
                            linestyle = ":",
