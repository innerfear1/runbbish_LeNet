...
def main():
    ...
    # 检查数据集的位置是否存在
    data_path = 'E:\VScode\python3/rubbish\Data/rubbishdata'
    ...
    optimizer = optim.Adam(model_LeNet.parameters(),lr=0.1)
    # 定义训练轮数
    epochs = 100
    # 基础准确率为0
    best_acc = 0.0
    ...
    for epoch in range(epochs):
        ...
        for step1,data in enumerate(train_bar):
            ...
            # 正向传播得出输出
            output = model_LeNet(images.to(device))
            # 按行最大输出对应的索引值
        ...
        with torch.no_grad():
             # 通过网络预测出结果
            val_output = model_LeNet(val_images.to(device))
            # 计算此参数用测试集图片的对应的损失值
            ...
            # 将对应的损失放入损失的列表中
            LeNet_loss.append(val_loss)
            # 将tensor类型数据转成float类型并加入准确率对应的列表中
            LeNet_acc.append(val_acc/val_labels.size()[0])

        # 如果准确率大于基准的准确率0，就保存此训练参数文件
        if (val_acc/val_labels.size()[0]) > best_acc:
            best_acc = val_acc/val_labels.size()[0]
            torch.save(model_LeNet.state_dict(),save_path)
    # 打印输出训练结束
    print('train_finished')

    # 定义和写入训练轮数的列表
    epoch_ls = list(range(1, epochs + 1))
    LeNet_acc = torch.tensor(LeNet_acc, device='cpu')
    LeNet_loss = torch.tensor(LeNet_loss, device='cpu')
    # 定义画图的画布大小
    fig, ax = plt.subplots(figsize = (7, 5))
    # 在画布中画出LeNet损失曲线
    LeNet_loss_plt, = ax.plot(epoch_ls, 
                             LeNet_loss, 
                             color = 'red', 
                             linewidth = 1.5, 
                             linestyle = ":", 
                             ...
                             label = 'LeNet损失')
    # 在画布中画出LeNet的准确率曲线
    LeNet_acc_plt, = ax.plot(epoch_ls,
                            LeNet_acc,
                            color = 'orange',
                            linewidth = 1.5,
                            linestyle = ":",
